...
class DoctorViewSet(viewsets.ModelViewSet):
    ...
    def reorder_display(self, request):
        ...
        if not doctor_id or direction not in {'up', 'down'}:
            return Response({'detail': 'doctor_id va direction (up/down) kerak.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        clinic = getattr(request.user, 'clinic', None)
        if request.user.is_clinic and (not clinic or doctor.clinic_id != clinic.id):
            return Response({'detail': 'Faqat o‘z klinikangiz doktorlarini tartiblashingiz mumkin.'}, status=status.HTTP_403_FORBIDDEN)

        if not doctor.clinic_id:
            return Response({'detail': 'Doktor klinikaga biriktirilmagan.'}, status=status.HTTP_400_BAD_REQUEST)

        visible_doctors = list(
            Doctor.objects.filter(clinic_id=doctor.clinic_id, is_active=True)
            .order_by('display_order', 'created_at', 'id')
        )
        ordered_ids = [item.id for item in visible_doctors]
        try:
            current_index = ordered_ids.index(doctor.id)
        except ValueError:
            return Response({'detail': 'Doktor faol ro‘yxatda topilmadi.'}, status=status.HTTP_400_BAD_REQUEST)

        swap_index = current_index - 1 if direction == 'up' else current_index + 1
        if swap_index < 0 or swap_index >= len(visible_doctors):
            return Response({'detail': 'Doktorni bu yo‘nalishda siljitib bo‘lmaydi.'}, status=status.HTTP_400_BAD_REQUEST)

        other_doctor = visible_doctors[swap_index]
        if other_doctor.id == doctor.id:
            return Response({'detail': 'Doktorni bu yo‘nalishda siljitib bo‘lmaydi.'}, status=status.HTTP_400_BAD_REQUEST)

        with transaction.atomic():
            first = Doctor.objects.select_for_update().get(id=doctor.id)
            second = Doctor.objects.select_for_update().get(id=other_doctor.id)
            first_order = first.display_order
            first.display_order = second.display_order
            second.display_order = first_order
            first.save(update_fields=['display_order', 'updated_at'])
            second.save(update_fields=['display_order', 'updated_at'])

        return Response({'detail': 'Doktor tartibi yangilandi.'}, status=status.HTTP_200_OK)
