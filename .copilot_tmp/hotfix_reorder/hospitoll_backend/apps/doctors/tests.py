...
class DoctorEmploymentLifecycleTests(APITestCase):
    ...
    def test_clinic_owner_can_reorder_doctors_for_public_listing(self):
        second_user = CustomUser.objects.create_user(
            username='doctor.order.second',
            email='doctor.order.second@example.com',
            password='Pass12345!',
            role='doctor',
            first_name='Second',
            last_name='Doctor',
        ...
        second_doctor = Doctor.objects.create(
            user=second_user,
            clinic=self.clinic_a,
            license_number='LIC-ORDER-002',
            pinfl='99887766554433',
            passport_id='CD7654321',
        ...
        initial_response = self.client.get(list_url, {'clinic': str(self.clinic_a.id)})
        self.assertEqual(initial_response.status_code, 200)
        initial_results = self._list_results(initial_response)
        self.assertEqual(str(initial_results[0]['id']), str(self.doctor.id))
        self.assertEqual(str(initial_results[1]['id']), str(second_doctor.id))

        reorder_url = reverse('doctor-reorder-display')
        reorder_response = self.client.post(reorder_url, {
            'doctor_id': str(self.doctor.id),
            'direction': 'down',
        }, format='json')
        self.assertEqual(reorder_response.status_code, 200)

        after_response = self.client.get(list_url, {'clinic': str(self.clinic_a.id)})
        self.assertEqual(after_response.status_code, 200)
        after_results = self._list_results(after_response)
        self.assertEqual(str(after_results[0]['id']), str(second_doctor.id))
        self.assertEqual(str(after_results[1]['id']), str(self.doctor.id))
        self.assertLess(int(after_results[0]['display_order']), int(after_results[1]['display_order']))
